...
void senddata(IPAddress ipdestination, String data){
  debug("Sending packet : " + data , 2);
  debug("IP : " + ipdestination.toString() , 2);
...
void udpservertest(){
  ...
  if (packetSize > 0) {
    ...
    debug("Received packet from : " + udp.remoteIP().toString() + " with size of : " + String(packetSize) + " end data : " + String(packetBuffer),2); 
...
void udpserver(void * parameter){
  for(;;) {
    ...
    vTaskDelay(3000 / portTICK_PERIOD_MS); // Ajout de vTaskDelay pour éviter de bloquer le processeur
  }
...
void wifiTask(void * parameter) {
  for (;;) {
    ...
    vTaskDelay(100 / portTICK_PERIOD_MS); // Délai pour permettre le basculement des tâches
  }
}
...
void setup() {
  ...
  // xTaskCreate(
  //     udpserver, /* Function to implement the task */
  //     "UDPserverTask", /* Name of the task */
  //     10000,  /* Stack size in words */
  //     NULL,  /* Task input parameter */
  //     1,  /* Priority of the task */
  //     &UDPserverTask);  /* Task handle. */ /* Core where the task should run */
...
void loop() {
  ...
  if (loopcounter <= 10){
    ...
    // RSSIValues rssiValues = scanAndGetRSSIs(ssidA, ssidB);
    // uint64_t elapsedMillis = millis() - startMillis;
